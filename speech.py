...
# Define poses for letters A-Z + space (your existing dictionary)
letter_poses = {
...
def spell_word(dxl_client, word):
    word = word.upper()
    for letter in word:
        ...
        time.sleep(2.0)  # Hold pose for 2 seconds

def listen_for_word(recognizer, microphone):
...
def main():
    try:
        ...
        while True:
            mode = input("Select input mode - (V)oice or (T)yping or (Q)uit: ").strip().upper()
            if mode == 'Q':
                print("[INFO] Exiting program.")
                break

            if mode == 'V':
                word = listen_for_word(recognizer, microphone)
                if word is None:
                    print("[INFO] Try again.")
                    continue
                spell_word(dxl_client, word)

            elif mode == 'T':
                word = input("Enter a word to spell (or Q to quit): ").strip()
                if word.upper() == 'Q':
                    print("[INFO] Exiting program.")
                    break
                spell_word(dxl_client, word)
                ...
                print("[WARN] Invalid mode selected, please choose 'V', 'T', or 'Q'.")
